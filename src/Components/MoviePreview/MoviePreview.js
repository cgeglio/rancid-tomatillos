  ...
  return (
    <article onClick={ () => saveSelectedMovieToStore(movie) } className='movie-preview-container'>
      ...
      <div className='ratings'>
        ...
        {movie.user_rating ? <div className='user-rating-bundle'><p className='preview-rating'>{movie.user_rating}</p><img src={lemon} className='rating-lemon'/></div> : <Link to={!user ? '/login' : `movies/${movie.id}`} className='preview-rating-button'>Rate</Link>}
      </div>
